...
const appControl = (() => {
  ...
  const genDefaultTask = () => {
    let defaultTask = Task(
      ...
      'tasktemple lets you organise your to do list by groups - or projects. Set due date, priority, and add notes'
    );
    return defaultTask;
  };
  ...
  function addObjectController(template) {
    ...
    if (!document.querySelector(`[name="${template}-notes"]`)) {
      //if there are no notes, implying project rather than task
